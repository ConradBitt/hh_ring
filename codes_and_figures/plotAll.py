...
fig, ax = plt.subplots(2,3, figsize=(20*cm,12*cm), gridspec_kw={'width_ratios':[5,2,2], 'height_ratios': [4,2] })
...
l01 = ax[0][1].legend(loc='center',fontsize="5", 
...
ax[0][2].scatter(x = last_lop, y = n, s = 0.1, color='blue', label=r'$\langle LOP_{n} \rangle$')
ax[0][2].scatter(x = mean_lop, y = n, s = 0.2, color='red', alpha=1, label=r'$\langle LOP \rangle$')
...
for axis in ax[:,0]:
    ...
    axis.set_xlim(2.2e4, 2.4e4)
    axis.set_xticklabels([str(x).replace('.',',') for x in axis.get_xticks()/1e3])
