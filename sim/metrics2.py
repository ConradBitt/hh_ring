...
@jit(nopython=True)
def param_local_order(spatial_phase_arr, delta):
    """
...
@jit(nopython=True)
def kuramoto_param_local_order(phases, delta):
    lop = np.zeros_like(phases.T, dtype=float64)
    i = 0
    for phase in phases.T:
        lop[i] = param_local_order(phase, delta)
        i += 1
    lop = lop.T
    return lop
...
def countNeuronsUnderThr(lop, thresholds):
    ...
    Returns:
        tuple: Uma tupla contendo:
            ...
            - Um array NumPy representando a última iteração das séries temporais.
